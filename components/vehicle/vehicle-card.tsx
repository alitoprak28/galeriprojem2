...
export function VehicleCard({ vehicle }: VehicleCardProps) {
  ...
  return (
    <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#13181f] transition duration-500 hover:-translate-y-1 hover:border-white/20">
      <div className="relative h-56 overflow-hidden sm:h-[31rem]">
        <Image
        ...
        <div className="absolute left-4 right-4 top-4 hidden flex-wrap items-start justify-between gap-2 sm:left-5 sm:right-5 sm:top-5 md:flex">
        ...
        <div className="absolute inset-x-4 bottom-4 hidden space-y-3 sm:inset-x-5 sm:bottom-5 sm:space-y-4 md:block">
      ...
      <div className="space-y-3 p-4 md:hidden">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap gap-2">
            <div className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-white">
              {vehicle.badge}
            </div>
            <StatusBadge status={status} />
          </div>
          <p className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-white/80">
            {vehicle.year}
          </p>
        </div>
        ...
        <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
          ...
          <h3 className="mt-2 text-[1.9rem] font-semibold leading-[0.95] text-white">{vehicle.model}</h3>
          <p className="mt-1 text-base text-white/60">{vehicle.series}</p>
          <p className="mt-3 text-sm leading-6 text-white/78">{vehicle.shortDescription}</p>
        </div>

        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-[18px] border border-white/10 bg-white/[0.03] p-3 text-center">
            <p className="text-[10px] uppercase tracking-[0.16em] text-white/45">KM</p>
            <p className="mt-2 text-sm font-semibold text-white">{formatKm(vehicle.km)}</p>
          </div>
          <div className="rounded-[18px] border border-white/10 bg-white/[0.03] p-3 text-center">
        ...
        <div className="flex items-end justify-between gap-4 rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.16em] text-white/45">Fiyat</p>
            <p className="mt-2 break-words text-[1.85rem] font-semibold leading-none text-accent">
              {formatPrice(vehicle.price)}
            </p>
          </div>
          <Link
